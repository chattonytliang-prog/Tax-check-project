...
describe('ruleEngine', () => {
  ...
  it('summarizes condition shapes for the UI', () => {
    ...
    expect(conditionSummary({ field: 'a', operator: '>=', value: 3, compareField: 'b', transform: 'absDiff' })).toBe('|a - b| >= 3')
    expect(conditionSummary({ field: 'a', operator: '>=', value: 3, transform: 'absDiff' })).toBe('|a - 0| >= 3')
    expect(conditionSummary({ field: 'privateAccountCollection', operator: '=', value: true })).toBe('个人账户收款 = 是')
    expect(conditionSummary({ field: 'privateAccountCollection', operator: '=', value: false })).toBe('个人账户收款 = 否')
    expect(conditionSummary({ field: 'annualRevenue', operator: '>', value: 5000000 })).toBe('年销售收入 > 5,000,000')
