...
describe('report period evidence sources', () => {
  ...
  it('formats type, period, and storage status without overstating evidence', () => {
    ...
    expect(reportPeriodEvidenceSourceStatus(stored)).toBe('原件已保存 · 解析完成 · 已入库 8 条')
    expect(reportPeriodEvidenceSourceStatus({ ...stored, stored: false })).toBe('原件未保存 · 解析完成 · 已入库 8 条')
    expect(reportPeriodEvidenceSourceStatus({ ...stored, recordCount: 0 })).toBe('原件已保存 · 解析完成 · 未形成标准记录')
    expect(reportPeriodEvidenceSourceStatus({ ...stored, stored: false, parseStatus: 'failed', recordCount: 0 })).toBe('原件未保存 · 解析失败 · 未形成标准记录')
  })
})
