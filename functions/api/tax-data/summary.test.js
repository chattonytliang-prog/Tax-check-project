...
describe('archive evidence counts', () => {
  it('counts registered originals separately from stored originals and linked records', async () => {
    ...
    expect(body.sourceFiles[5]).toMatchObject({ stored: true, recordCount: 0, reviewNote: expect.stringContaining('标记为已解析，但未形成标准记录') })
    ...
    expect(body.sourceFiles[8]).toMatchObject({ stored: false, recordCount: 0, reviewNote: expect.stringContaining('原件未保存且未形成标准记录') })
