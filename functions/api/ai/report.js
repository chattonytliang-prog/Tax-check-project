...
function removeInternalReportArtifacts(content) {
  return String(content || '')
    ...
    .replace(/^规则执行覆盖[：:]?[^\n]*(?:\n|$)/gm, '')
    .replace(/^规则执行情况[：:]?[^\n]*(?:\n|$)/gm, '')
    .replace(/已执行规则风险结论/g, '风险结论')
    .replace(/已执行规则/g, '本次检查')
...
2. 报告中的风险事项只能来自“风险事项”列表，不得新增或删除。
3. AI 数据复核中发现的疑点，只能写入“数据复核提示”或“观察项”，不能写成已命中风险。
4. 保留风险提示性质，不要承诺最终税务处理结论。
5. 输出中文纯文本，不要使用 Markdown 代码块。
6. 结构包含：企业基本情况、风险结论、资料覆盖范围、资料不足暂未判断事项、AI 数据复核提示、分税种风险摘要、重点风险事项、风险明细与整改建议、整改优先级、建议补充资料、免责声明。
7. 语气专业、审慎、可执行。
...
12. 面向客户的报告中禁止输出“规则、规则引擎、规则库、已执行规则、未执行规则、规则执行覆盖”等系统内部表述，也禁止输出内部编号、issueId、code、内部字段名或类似“smallProfitEnjoyed=true”的条件表达式。
13. 必须明确这是“基于已提供资料的风险初筛”，不得称为完整税务体检；资料覆盖度不得表述为全部资料完整度。
14. 因资料不足暂未判断的事项必须保留为资料缺口，不得当作低风险或无风险。
    ...
    const response = await fetch(DEEPSEEK_API_URL, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: '你是一名严谨的中国企业税务风险管理顾问，擅长把已识别的风险转化为可复核、可整改的客户报告。客户报告不得披露内部检测机制。涉及日期、期间、成立时长时，必须服从系统计算事实。',
          },
          {
            role: 'user',
